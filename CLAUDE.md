...
**IdP routing:** `handle_idp()` in `webservice.cpp` dispatches the IdP routes the same way `handle_sso()` does — parse, call the transport-agnostic handlers in `modules/module/inhouseidp/inc/idp_*.hpp`, render the `SsoHttpResult` (reused from v1.0 — the contract is identical). The IdP-side cookies live at `Path=/api/v1/idp/` so they're never sent to the marvel app: `xpmile_idp_session` (12 h IdP login session) and `xpmile_idp_pending` (10 min, ties `/login` back to the originating `/authorize`). Wired routes today (slices 1+2): `/.well-known/openid-configuration`, `/api/v1/idp/jwks`, `/api/v1/idp/authorize`, `/api/v1/idp/userinfo`, `/api/v1/idp/end_session`. Stubbed at 501 pending slice 3: `/api/v1/idp/login`, `/api/v1/idp/token`, `/api/v1/idp/password/{reset_request,reset_confirm}`. Slice 3 needs the four `IPasswordVerifier`/`IPasswordHasher`/`IJwtSigner`/`IEmailSender` production impls — `WsdbJwtSigner` already exists; the rest will land alongside the legacy-login refactor (Phase K).
