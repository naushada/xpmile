...
Up to four core services: `mongodb` (built locally from `docker/Dockerfile.mongo`, ~30 s — OR for shape-A operators, pulled per-arch from `docker.io/naushada/xpmile-mongo:{7,4.4.18,latest}` via the one-file `install-agent.sh`), `wsdbagent` (pulled from `docker.io/naushada/xpmile-wsdbagent:latest` — CI-published per deploy; pin with `WSDBAGENT_IMAGE=...:<sha>`), the optional `wsdbagent-idp` (same image, dedicated to the IdP Heroku app — included only when `IDP_SERVER_HOST` is set in `.env`), and `xpmile-cert-watcher` (alpine sidecar; `docker.io/library/alpine:3.19`). The watcher md5sums `./certs/cloud-issued/innertls/` every 5 s and POSTs a restart to *both* agents via the host podman socket on any change — the idp agent's restart endpoint returning 404 is treated as benign (marvel-only stack). Same image to both Heroku apps means one InnerTLS CA on-prem and one shared client cert family — `refresh-certs` rotates the single set; the cert-watcher fans the restart out. Full rotation story in `docs/ws-db-agent.md`.

**InnerTLS cert family source** — `docker/Dockerfile` (uniservice) mints a fresh CA + server cert + matching client cert family per build (PR #8). The cloud-side image bakes in the CA + server cert. The wsdbagent image now bakes in the matching client cert family too (PR #44 — `Dockerfile.wsdbagent` has a `FROM ${UNISERVICE_REF} AS uniservice-source` stage; the runtime stage `COPY --from=uniservice-source /opt/xAPP/granada/agent-certs/ /opt/wsdbagent/baked-certs/`). CI serializes `wsdbagent` after `uniservice` (`needs: [bootstrap, test, uniservice]`) and passes `UNISERVICE_REF=${{ env.UNISERVICE_IMAGE }}:${{ github.sha }}` so wsdbagent gets THIS workflow run's matching certs, not whatever `:latest` happens to be. `./run-agent.sh refresh-certs` and the standalone `install-agent.sh` both extract from `/opt/wsdbagent/baked-certs/` — ~10 MB pull of an image the operator needs anyway, instead of the old ~500 MB uniservice pull just to scrape three cert files.
...
**Implementation traps — surfaced by the first live federated-login walkthrough; do not re-introduce:**
- **`idp.idp_signing_keys` schema split.** Vaadin writes `_id = new ObjectId()` (auto-gen) and stores the domain `kid` in its own `kid` field. Any reader of the collection (cloud-side `idp_jwks.cpp`, on-prem `sign_jwt_on_prem.cpp`) MUST project `{"_id":0, "kid":1, …}` and read the kid from `kid`. Reading `_id` as a string crashes the worker with `nlohmann::json::type_error.302` (it's a `{"$oid":"…"}` object after canonical-JSON serialisation). The sibling collection `idp.idp_clients` is the opposite — `_id` IS the clientId, intentionally; keep the distinction.
- **`MicroService::handle_idp`'s render switch.** The switch in `webservice.cpp` MUST branch on every 4xx/5xx the IdP handlers can return (302, 400, 401, 403, 404, 500, 503 today). Anything that falls into `default:` is rendered as **HTTP 200** with the JSON error body — which on a `/token` 500 reached marvel as a fake 200, marvel parsed no `id_token` field, and the actual signing failure was buried under the generic `?error=callback_failed`. Same pattern would apply to `handle_sso`'s switch. **Adding a new status code requires an explicit `case` or the failure becomes invisible.**
- **OIDC discovery URL.** Per OIDC core §4 + RFC 8414, the discovery URL is `<issuer>/.well-known/openid-configuration` (path-concatenated). Our issuer carries a path (`https://idp-…/api/v1/idp`), so RPs hit `https://idp-…/api/v1/idp/.well-known/openid-configuration`. The wire adapter accepts both this and the bare root for tooling.
- **wsdbagent image is pulled only at `up`, not at cert-watcher restart.** After a CI release that touches `Dockerfile.wsdbagent`, the on-prem operator must explicitly `podman pull docker.io/naushada/xpmile-wsdbagent:latest && ./run-agent.sh stop && ./run-agent.sh start` to pick up the new binary. `./run-agent.sh refresh-certs` rotates only the InnerTLS cert family; it does NOT re-pull. Full troubleshooting entry: [`docs/inhouse-idp.md`](docs/inhouse-idp.md) → *agent-wsdbagent / agent-wsdbagent-idp is running an outdated image after a Heroku release*.
