...
- **[runc-pull (xpmile-pull)](docs/design/runc-pull/USAGE.md)** — the in-house Docker Hub puller shipped in v1.3.0. `runcpull::PullOrchestrator` C++ library + standalone `xpmile-pull` binary + per-arch operator tarball ([v1.3.0 release](https://github.com/naushada/xpmile/releases/tag/v1.3.0)). Replaces `skopeo` + `umoci` in the runc install path. See [`docs/design/runc-pull/`](docs/design/runc-pull/) for the design and TDD plan.
...
| [`docs/design/runc-pull/`](docs/design/runc-pull/) — [design](docs/design/runc-pull/runc-pull-design.md), [TDD plan + status](docs/design/runc-pull/runc-pull-tdd-plan.md), [USAGE](docs/design/runc-pull/USAGE.md) | In-house Docker Hub puller (`xpmile-pull`) that replaces `skopeo` + `umoci` in the runc install path. All 8 phases shipped in v1.3.0 (141 GTest across `runcpull::` + the `main.cpp` wiring + per-arch operator tarball on the [release page](https://github.com/naushada/xpmile/releases/tag/v1.3.0)) |
