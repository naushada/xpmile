...
- **[runc-pull library + binary plan](docs/design/runc-pull/USAGE.md)** — the in-house Docker Hub puller (`runcpull::PullOrchestrator` C++ library shipping today; standalone `xpmile-pull` binary + release tarball pending Phase H). Replaces `skopeo` + `umoci` in the runc install path. See [`docs/design/runc-pull/`](docs/design/runc-pull/) for the design and TDD plan.
