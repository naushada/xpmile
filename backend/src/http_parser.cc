...
static std::string multipart_boundary(const std::string &ct) {
  ...
  if (pos == std::string::npos)
    return {};
  std::string b = ct.substr(pos + 9); // skip "boundary="
  auto semi = b.find(';');
...
// Transfer-Encoding is stripped first so this receives the raw compressed
// bytes.
static std::string decompress(const std::string &in,
                              const std::string &encoding) {
  // gzip: windowBits = 15+16; deflate: windowBits = 15+32 (auto-detect zlib or
  // gzip wrapper)
...
void Http::parse_mime_header(const std::string &in) {
  ...
  while (std::getline(input, line)) {
    ...
    // Skip the ": " separator; guard against malformed lines with no space
    // after colon
    std::string value =
...
std::size_t Http::message_length(const std::string &buf) {
  ...
  // Rule 2: explicit Content-Length — covers plain, gzip, deflate, and
  // multipart/form-data bodies when the sender includes Content-Length.
  if (!cl.empty()) {
    try {
  ...
  // Rule 3: multipart/form-data without Content-Length — delimited by the
  // closing boundary marker "--{boundary}--" (RFC 2046 §5.1).
  {
    ...
    if (!boundary.empty()) {
      ...
      auto end_pos = buf.find(terminal, header_len);
      if (end_pos == std::string::npos)
        return 0; // closing boundary not yet received
      std::size_t end = end_pos + terminal.size();
      // Account for optional trailing CRLF after the closing boundary line.
...
std::string Http::get_body(const std::string &in) {
  ...
  // Transfer-Encoding is the outermost framing layer — decode it first.
  if (te == "chunked") {
    raw = decode_chunked(in.substr(body_offset));
  } else if (!cl.empty()) {
    // Explicit Content-Length covers plain, gzip/deflate, and multipart bodies
    // when the sender provides the header.
    if (body_offset > 0)
      raw = in.substr(body_offset, static_cast<std::size_t>(std::stoul(cl)));
  } else {
    // Multipart/form-data without Content-Length: slice up to and including
    // the closing boundary "--{boundary}--" (RFC 2046 §5.1).
    std::string boundary = multipart_boundary(ct);
    if (!boundary.empty()) {
      std::string terminal = "--" + boundary + "--";
  ...
  // (Multipart bodies are not typically compressed at the message level.)
