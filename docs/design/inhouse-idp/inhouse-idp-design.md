...
- **Discovery URL served at BOTH `/.well-known/openid-configuration` AND `/api/v1/idp/.well-known/openid-configuration`** (PR #35, surfaced during the first live login attempt). OIDC core §4 / RFC 8414 says the discovery URL is `<issuer>/.well-known/openid-configuration` (path-concatenated). Our issuer carries `/api/v1/idp`, so RP libraries (including marvel's `sso::fetch_discovery`) hit the path-prefixed URL. The original implementation only matched the bare root, so production discovery 501'd. Wire adapter now accepts both.
- **`handle_idp`'s render switch must enumerate every 4xx/5xx** (PR #36, same live-login session). The earlier switch had only 302/400/401 cases; everything else (notably 500 from `/token` signing failures) fell into `default:` → `build_responseOK`. Marvel saw HTTP 200 with the JSON error body, parsed "no id_token field", and reported the generic `?error=callback_failed` — completely hiding the real signing failure. The switch now branches on 302/400/401/403/404/500/503. Adding new IdP error codes requires a new case.
- **Signing-key schema split** (`idp.idp_signing_keys`): `_id` is `new ObjectId()` (auto-gen), domain kid is in its OWN `kid` field. The cloud-side reader (`idp_jwks.cpp`) AND the on-prem reader (`sign_jwt_on_prem.cpp`) BOTH had to be fixed (PRs #33 and #36) to project `{"_id":0, "kid":1}` and read the kid from `kid`. Reading `_id` as a string crashes the worker on the canonical `{"$oid":"…"}` serialisation. The sibling collection `idp_clients` is the opposite — `_id` IS the clientId by design. Keep the distinction in mind for any future code that touches either collection.
...
9. **Parameterise the two database names** (`xpmile`, `idp`). Both are string literals across `docker/mongo-init.js`, the C++ `idp::*` constants (`kDb = "idp"`), the Java `IdpSigningKeyService` / `IdpClientService` (`IDP_DATABASE = "idp"`), `scripts/migrate-account-split.py`, and the Phase K cross-DB read in `webservice.cpp`. Pulling them into env-driven constants (with the current literals as defaults) would let a single MongoDB cluster host multiple xpmile installs without name collisions. Out of v1 scope — touches ~12 files and invalidates the design's "dual-DB" terminology in subtle ways since the names lose meaning.
10. **Account-creation dual-write** (`handle_account_POST` + `handle_account_PUT`). Today, marvel's account-creation handler writes the new account to `xpmile.account` only — the auth fields (passwordHash) never reach `idp.account`. Phase K's migration script + cross-DB login fallback covers EXISTING accounts at migration time, but accounts created via Vaadin's **Accounts** view post-Phase-K need the operator to drop `xpmile.schema_version` + re-run the migration to surface in `idp.account`. The proper fix: refactor the two handlers to split the write — business fields (+ `loginCredentials.accountCode` as the join key) to `xpmile.account`; auth fields (passwordHash, email, name, role) directly to `idp.account`. Atomic-ish ordering: write `idp.account` first (so legacy login's fallback always has the hash); fall back to retry/abort on the marvel-side write. Tests need to cover the dual-write + the partial-failure cleanup. ~2-3 hours; tracked as a follow-up to Phase K.
