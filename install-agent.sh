...
ExecStart=$(command -v podman-compose) -f docker-compose.agent.yml $IDP_PROFILE_ARG up -d
...
${C_BOLD}Routine maintenance${C_OFF}
  ...
  Start:          systemctl --user start xpmile-agent    # or: $COMPOSE_CMD -f $INSTALL_DIR/docker-compose.agent.yml ${IDP_SERVER_HOST:+--profile idp }up -d
  Restart:        systemctl --user restart xpmile-agent
